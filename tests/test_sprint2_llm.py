...
class TestLLMMetricsCollector:
    ...
    async def test_error_tracking(self, collector):
        ...
        await collector.end_call(
            "ollama", latency=0.5, success=False, error_type="timeout"
        )

        await collector.start_call("ollama")
        await collector.end_call(
            "ollama", latency=0.3, success=False, error_type="timeout"
        )
...
class TestOllamaClient:
    ...
    async def test_ollama_complete_mock(self, client):
        ...
        mock_response = {"response": '{"decision": true, "confidence": 0.85}'}

        with patch.object(
            client._get_ollama_session(),
            "post",
            return_value=AsyncMock(
                status=200, json=AsyncMock(return_value=mock_response)
            ),
        ):
...
class TestIntegration:
    ...
    async def test_end_to_end_decision_flow(self):
        ...
        # Mock the LLM call
        mock_response = (
            '{"decision": true, "confidence": 0.8, "reasoning": "Test", "urgency": 0.5}'
        )
